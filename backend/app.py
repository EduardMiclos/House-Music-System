...
def update_cache(song_cache_json, song_objects):
    for song_obj in song_objects:
        song_cache_json[song_obj.id] = song_obj
...
def play_song():
    ...
    if song_id is not None and song_id > 0:
        ...
        elif song_id in recently_played_songs_cache:
            song = recently_played_songs_cache[song_id]
        elif song_id in searched_songs_cache:
            song = searched_songs_cache[song_id]
...
def pause_song():
    ...
    # tbd send socket message to update state [maybe abstract the emit method]
    return jsonify({"status": "paused", "song": ms.music_manager.current_song.to_json()}), 200
